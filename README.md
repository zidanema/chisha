...
- **FastAPI 20 端点** = D-069 基础 13（推荐 6 + V1.1 反馈 7） + D-076 1（`/api/long_term_prefs/refresh`） + D-077 6 sandbox（`/api/sandbox/{init,advance,reset,disable,state,inspect}`），单 JSON 文件落盘（[IMPL_LOG D-069](docs/IMPLEMENTATION_LOG.md#d-069-执行记录--fastapi-v1--v11-后端-13-端点联调--codex-review-修复) + D-076/074）
- **调试台 (两套并存)**：① 老 vanilla HTML `chisha/static/debug.html` 走 `:8765/debug` instrumented 管道, L1/L2/L3/Final 四段 + 16 维 breakdown + LLM payload 可见 + combo 追溯 + mood 三栏对比 ([D-039](docs/DECISIONS.md#d-039));② 新独立 SPA `apps/debug-ui/` 走 `:5174` (Vite + React 18 + TS, V12 DAG sticky + 5 主题 + Refine 二轮 trace + 追溯 Tab, 7 phase Codex-reviewed, [D-075](docs/DECISIONS.md#d-075-apps-debug-ui-独立-spa-不并入-apps-web))
