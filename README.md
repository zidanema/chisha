...
Step 2 用户自用验证现在可走 sandbox 一次会话压缩到分钟级（开沙盒 → 点"下一天" → 真实写反馈 → inspect 看 L1 抽取产物），不必等真实日历日 → Phase 1 同事推广。D-078/D-078.1/2/3 + D-076.1 修补后真实 LLM 两轮演练已绿（low_oil boost 抽出 + Day 5 油 -7.4%; spicy boost 抽出 + Day 5 LLM 显式 "顶格命中追辣 boost"）—— **L1 (LLM 抽取) → L2 (taste_match_bonus) → L3 (prompt 注入"行为信号") 三层贯通**。Codex S2 audit 抓出 refine 二轮 root threading 漏点已修。
...
| [docs/BACKLOG.md](docs/BACKLOG.md) | 待办池 · 已知但当前不解决的 bug / feature / idea | 发现新 bug 但不立刻修时记一笔 |
...
- **L1 长期反馈层**（[D-076](docs/DECISIONS.md#d-073-l1-长期反馈层重构--砍伪-l1--llm-抽取-v1x)）：`chisha/l1_extractor.py`（V1.1 反馈预聚合 → claude_code_cli text + JSON prompt + parse/validate/retry） + `chisha/l1_prefs.py`（6 token enum 校验 + 损坏 fail-open）+ `prompts/l1_extract.md`（system prompt + 3 反例）+ `scripts/bootstrap_l1_from_legacy.py`（兜底冷启动）
- **Sandbox Time-Travel 模式**（[D-077](docs/DECISIONS.md#d-074-sandbox-time-travel-模式-v1x)）：`chisha/clock.py`+`chisha/sandbox.py`（虚拟时钟 + state）+ `chisha/data_root.py`（7 路径派生隔离）+ `/api/sandbox/*` 6 端点 + 前端 `apps/web/src/components/SandboxBar.tsx` + ProfilePage 入口 + Inspect Drawer
...
- **FastAPI 23 端点** = D-069 基础 13（推荐 6 + V1.1 反馈 7） + D-076 1（`/api/long_term_prefs/refresh`） + D-077 6 sandbox（`/api/sandbox/{init,advance,reset,disable,state,inspect}`） + D-079 3 trace（`/api/debug/sessions` GET list + `/api/debug/sessions/{sid}` GET detail + `/api/debug/what_if` POST），单 JSON 文件落盘（[IMPL_LOG D-069](docs/IMPLEMENTATION_LOG.md#d-069-执行记录--fastapi-v1--v11-后端-13-端点联调--codex-review-修复) + D-076/074）
- **调试台 (两套并存)**：① 老 vanilla HTML `chisha/static/debug.html` 走 `:8765/debug` instrumented 管道, L1/L2/L3/Final 四段 + 16 维 breakdown + LLM payload 可见 + combo 追溯 + mood 三栏对比 ([D-039](docs/DECISIONS.md#d-039));② 新独立 SPA `apps/debug-ui/` 走 `:5174` (Vite + React 18 + TS, V12 DAG sticky + 5 主题 + Refine 二轮 trace + 追溯 Tab + **Replay / What-if / Live 三模式** [D-079], 7 phase Codex-reviewed, [D-075](docs/DECISIONS.md#d-075-apps-debug-ui-独立-spa-不并入-apps-web))
- **Trace 持久化**：`logs/recommend_trace/{sid}.json` 一次推荐一个文件（sandbox 派生 `logs/sandbox/recommend_trace/`），含 L1 全 drops + L2 16 维 breakdown + L3 LLM raw + `__frozen` 冻结上下文（What-if 重跑自包含, 不读 runtime state）。详见 [D-079](docs/DECISIONS.md#d-079-推荐链路-trace-持久化--debug-三模式-replay--what-if--live) + [docs/DEBUG_REPLAY_DESIGN.md](docs/DEBUG_REPLAY_DESIGN.md)
...
│   ├── web_api.py             # FastAPI 端点 (D-069 13 + D-076/074 sandbox/refresh)
│   ├── clock.py + sandbox.py  # D-077 虚拟时钟 + sandbox state
│   ├── data_root.py           # D-077 7 路径派生 (prod/sandbox 切换)
│   ├── l1_extractor.py        # D-076 L1 LLM 抽取 (text + JSON parse/retry)
│   ├── l1_prefs.py            # D-076 prefs 读写 + 6 token enum 严格校验
│   ├── long_term_prefs.py     # D-043 旧伪 L1, DEPRECATED (D-076 PR-0.5)
