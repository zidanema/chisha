...
- **FastAPI 20 端点** = D-069 基础 13（推荐 6 + V1.1 反馈 7） + D-073 1（`/api/long_term_prefs/refresh`） + D-074 6 sandbox（`/api/sandbox/{init,advance,reset,disable,state,inspect}`），单 JSON 文件落盘（[IMPL_LOG D-069](docs/IMPLEMENTATION_LOG.md#d-069-执行记录--fastapi-v1--v11-后端-13-端点联调--codex-review-修复) + D-073/074）
...
│   ├── web_api.py             # FastAPI 端点 (D-069 13 + D-073/074 sandbox/refresh)
│   ├── clock.py + sandbox.py  # D-074 虚拟时钟 + sandbox state
│   ├── data_root.py           # D-074 7 路径派生 (prod/sandbox 切换)
│   ├── l1_extractor.py        # D-073 L1 LLM 抽取 (text + JSON parse/retry)
│   ├── l1_prefs.py            # D-073 prefs 读写 + 6 token enum 严格校验
