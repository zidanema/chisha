...
并发边界 (产品拍板接受, 接 D-099 TOCTOU 先例 + 提案 §已知坑 2): 迁移是**一次性**显式操作
...
#   4. profile.yaml 存在且**内容非 template** (含 zone 占位 `<YOUR_LUNCH_ZONE>` = 模板; 缺即真数据)
#
# Rule 4 关键: 老的 dev checkout 里 profile.yaml 可能是用户真实数据 (含名字/zone/口味),
# 不含 zone 占位; 而 ship 出去的 profile.yaml = 占位模板. 所以 "profile.yaml 内容含
# `<YOUR_LUNCH_ZONE>` 占位 = 模板, 不算 legacy; 内容无占位 = 真个人数据, 算 legacy 触发迁移".
# 反例: dev 用户拉过 repo, profile.yaml 是真数据 + 从未跑过 chisha
# (无 logs/feedback/prefs) — 单靠 1-3 漏判, Rule 4 兜底.
#
# 真空模板把 name/city/goal/taste 改 null (去掉 <YOUR_NAME>), 哨兵改用仍在模板里
# 的 zone 占位 <YOUR_LUNCH_ZONE> (onboard 写 state 时才替换, install_root 模板恒含它)。
_PROFILE_TEMPLATE_MARKER = "<YOUR_LUNCH_ZONE>"   # 真空模板占位字段 (与 profile.yaml zones 一致)
