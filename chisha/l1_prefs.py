...
- enum 限定 token 词表为 score.py taste_match_bonus 已支持的 6 个 (D-072 边界:
  词表扩展 = 改打分逻辑, 走独立决策 + baseline_l2_snapshot 守门, 不在 D-076 内)
...
#
# D-076.1: boost 方向扩 spicy / sweet_sauce, 与对应 penalty 形成对称.
# 触发: 演练实测发现"用户偏好辣"等正向口味信号只能进 regularities_freetext, 不进
# 打分链路 → 重度吃辣用户的核心口味偏好无法被系统消费. 不加 processed_meat /
...
# 兼容别名 → canonical (D-076 §2.2 Codex Q1)
