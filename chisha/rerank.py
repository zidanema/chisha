...
# D-047 矩阵实测 (lunch/want_light, sonnet+opus × top30/60/100 × 3
...
# 空画像默认文案 (consumer-onboarding 节 3b): taste_description=null/空 →
# 显式告知 L3 "口味未知", 仅按方法论 + 历史反馈 + context 排序, 不臆测偏好 (防 §3.5 占位污染).
_TASTE_EMPTY_PROMPT = "(未设: 仅按方法论 + 历史反馈和当天 context 排序, 不臆测口味)"


def render_taste_for_prompt(profile: dict) -> str:
    """taste_description 渲染给 L3 prompt: null/空白 → 显式"未设"文案; 否则原文 strip。

    单一权威源 (节 3b): _profile_block + 任何向 LLM 透传口味的地方都走这里, 防"空画像被当
    成真实口味"或"模板示例被当真"两种污染。
    """
    taste = profile.get("taste_description")
    if taste is None or (isinstance(taste, str) and not taste.strip()):
        return _TASTE_EMPTY_PROMPT
    return taste.strip()
...
def _profile_block(
    ...
    lines.extend([
        f"口味描述: {render_taste_for_prompt(profile)}",
...
def build_payload(
    ...
    payload: dict[str, Any] = {
        ...
        "profile": {
            # 空画像归一 None→"" (debug/trace payload 保持 str 类型; LLM 文案走 _profile_block).
            "taste_description": profile.get("taste_description") or "",
