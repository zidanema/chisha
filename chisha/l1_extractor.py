...
def aggregate_inputs(
    ...
    Args:
        ...
        today: 当日, None = clock.today(root) (D-074 PR-1a 第 12 处时间注入修补,
...
def _default_llm_call(prompt: str, system: str, profile_llm: dict | None) -> str:
    ...
    返回纯文本 (content). 不传 tools, claude_code_cli 才能用 (D-073 拍板 1A).

    D-075 修补: 原写的是 `llm_client.call`, 实际只存在 `call_text` (D-047 改名
    后 L1 没跟上). 之前 based_on_meals 永远卡在 1 (D-075 时钟 bug) 掩盖了这处.
    """
...
def extract_prefs(
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        ...
        # 注入元数据 (D-074 Codex S3: extracted_at 走虚拟时钟)
        from chisha import clock
