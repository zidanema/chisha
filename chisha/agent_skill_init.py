...
chisha onboard --zone shenzhen-bay        # 初始化 profile + skill + dry start
```

之后任意目录 Claude Code 触发"今天中午吃啥"即走本 skill.

## 命令

所有命令任意目录跑 (one-shot, 跑完即退, 输出 JSON 到 stdout). state 落 `~/.chisha/`:
...
```
chisha agent start --meal lunch --context "想吃辣的别太贵"
```
...
```
chisha agent resolve-intent --id <rid> --intent '{"correlation_id":"<抄 spec.correlation_id>","payload":<你产出的 intent 对象>}'
```
...
```
chisha agent apply-rerank --id <rid> --response '<信封 json>'
```
...
```
chisha agent choose --id <rid> --card <card_id> --action accept
```
...
```
chisha agent start --meal lunch --context "再辣点" --from <rid>
```
...
- `doctor` 自检: `chisha doctor` (sandbox 全局启用时 CLI 会拒绝跑, 先去 sandbox-lab disable).
