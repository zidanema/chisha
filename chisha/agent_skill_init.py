...
全局 PATH。**单一源**: build_skill_bundle / check_dist_manifest 都调 `_claude_code_skill_md`
取这份 SKILL.md, 故此函数是 bundle SKILL.md / `chisha skills add` / dist 校验的共同源
(函数名保留向后兼容, 内容已 **host-agnostic** — 见下).

**host-agnostic (Codex 兼容)**: 同一份 SKILL.md 被 Claude Code (`~/.claude/skills/`)
与 Codex (`~/.codex/skills/`) 共读 —— 两宿主都按 frontmatter `name`+`description` 自动路由
触发, 且都自动读 skill 根的 `SKILL.md`。故不做 dual-variant: 协议层通用, **唯一按宿主分支
的是「怎么把候选摆给用户选」这层呈现** (标 ⎇ 的步骤: Claude Code 用 AskUserQuestion, OpenClaw 用
feishu_ask_user_question, Codex 等用文本编号列表; do_llm 一律 inline 直跑, 不起 subagent)。`init_skill`/`skills add` 仍是
Claude Code 专属的便捷写盘入口 (Codex 走纯 clone 整包, 不用它)。
"""
...
def _claude_code_skill_md() -> str:
    """host-agnostic 单循环触发器 SKILL.md (D-105 形态B + Codex/OpenClaw 兼容).

    命令指向 bundle 内 scripts/chisha wrapper (路径 = 本 skill 文件夹内, 不依赖全局 PATH;
    Claude Code 在 `~/.claude/skills/chisha/`, Codex 在 `~/.codex/skills/chisha/`, OpenClaw 在 `~/.openclaw/skills/chisha/`)。
    frontmatter (name/description/触发词) 各宿主通用, 保证自动触发不变。

    D-118: 通道适配回归"寻常 skill" —— chisha 不再带飞书 channel/host/plugin, 呈现层由宿主用
    **自己的 ask-user-question 工具**完成 (Claude Code `AskUserQuestion` / openclaw-lark
    `feishu_ask_user_question` / 纯文本), 两者契约同构。唯一按宿主分支的仍是「怎么摆给用户」那层 (标 ⎇)。"""
    return r'''---
name: chisha
...
def _codex_openai_yaml() -> str:
    """Codex UI chip 元数据 (`agents/openai.yaml`) — bundle 根附带, **单源在此**.

    Codex (Codex.app / codex CLI) 读 `agents/openai.yaml` 作 skill 列表/chip 的人面文案;
    Claude Code 不认这个文件、直接忽略 (无害)。icon 字段按 Codex spec 为可选 —— 不造资源,
    只给 display_name/short_description/default_prompt (与 SKILL frontmatter 对齐)。"""
    return (
        "interface:\n"
        '  display_name: "今天吃点啥 (chisha)"\n'
        '  short_description: "按你的吃法配几组外卖, 扫一眼挑一个"\n'
        '  default_prompt: "中午吃啥"\n'
    )
...
def init_skill(agent: str, root: Path, *, force: bool = False, dest: Path | None = None) -> int:
    ...
    注: 这是 **Claude Code 专属**的便捷写盘入口 (只落 SKILL.md); Codex 走纯 clone 整包
    进 `~/.codex/skills/chisha/`, 不经此函数 (bundle 已自带 host-agnostic SKILL.md)。
    """
    ...
    if target.exists() and not force:
        # (PRD §3.5 #2): 已存在 SKILL.md 是良性的 (bundle 自带 / 先前已装) —
        # 返回 ok=true status=skipped (而非 EXISTS error), 否则严格照 verify 契约的 agent
        # 会误判 onboard 失败. --force 才覆盖.
        print(json.dumps({
            "ok": True,
            "status": "skipped",
            "agent": agent,
            "path": str(target),
            "hint": "SKILL.md 已存在 (bundle 自带或先前已装), 跳过写入; --force 覆盖.",
        }, ensure_ascii=False))
        return 0
