...
D-105 形态B (自包含 skill 分发): SKILL.md 从 "uv tool install + 全局 chisha" (形态A)
...
本 skill **自包含**: 代码 + 数据 + vendored 依赖都在本文件夹, 拷进 `~/.claude/skills/chisha-meal/` 即用, **零全局安装、运行期零联网**. 命令统一走 bundle 内 wrapper (下面记作 `CHISHA`):
...
skill 文件夹拷到 `~/.claude/skills/chisha-meal/` 即装好 (你正在读它 = 已就位). 初始化 profile + 自检:

```
python3 ~/.claude/skills/chisha-meal/scripts/chisha onboard --zone shenzhen-bay
```
...
- 上面 `CHISHA` 是占位, 实跑替成 `python3 ~/.claude/skills/chisha-meal/scripts/chisha`.
...
def init_skill(agent: str, root: Path, *, force: bool = False, dest: Path | None = None) -> int:
    ...
    print(json.dumps({
        ...
        "next": [
            ...
            "先跑 `python3 ~/.claude/skills/chisha-meal/scripts/chisha doctor` 自检环境 "
