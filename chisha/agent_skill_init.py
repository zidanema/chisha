...
**你**按 system + json_schema 产出一个 intent JSON 对象. 没自信映射进 slot 的诉求写进 `raw_understanding` (chisha 会当用户可见提示弹出). **不要回传 raw_text** (chisha 自己注入原话).

把 intent JSON 交回:

```
uv run python -m chisha.agent_cli resolve-intent --id <rid> --intent '<intent_json>'
```
...
**你**读候选, 按 system 规则挑 5 条 (R1: 3 exploit + 2 explore; refine 轮: 全 exploit), 产出:
...
```
uv run python -m chisha.agent_cli apply-rerank --id <rid> --response '<rerank_json>'
```
...
- 每步都把 `recommendation_id` 透传; correlation_id 由 chisha 管, 你照信封执行即可.
