...
def _list_zone_names(data_dir: Path) -> set[str]:
    """枚举 data_dir 下含 restaurants.json 的子目录名 (跳 dot 前缀: 版本目录/临时目录/update-state 不是 zone)."""
    if not data_dir.is_dir():
        return set()
    return {
        p.name for p in data_dir.iterdir()
        if not p.name.startswith(".") and p.is_dir() and (p / "restaurants.json").exists()
    }
...
def _resolve_zone_dir(zone: str, install_root_p: Path) -> Path:
    ...
    if user_has and install_has and not same_path:
        # 更新机制 (spec §4.6): canonical zone (zone_id ∈ install manifest) 按 data_version 仲裁,
        # state **严格更新**才胜; 否则 bundle 胜 (装了更新 bundle 后透明压过陈旧 state)。
        # 自定义同名 zone (不在 manifest) 维持 fail-loud 护栏不变 (无安全回退)。
        # codex Q3 (declined w/ reason): load_zones 每轮推荐重读 install manifest — 仅在 state+install
        # 同时有该 zone (即跑过 update) 时发生, 是「每次 recommend 一次小文件读」非内循环; 不缓存是
        # 刻意的 (spec §4.6: 长驻 debug_server 更新后下次 recommend 即生效 / 测试切 install_root 不串扰)。
        from chisha import manifest as _mfst
        install_zones = {z["zone_id"]: z for z in _mfst.load_zones(install_root_p)}
        if zone in install_zones:
            from chisha import zone_marker as _zm
            _m = _zm.read_marker(user_dir) or {}                            # 跟随 symlink 读版本目录 marker
            state_dv = _zm.valid_data_version(_m.get("data_version"))       # 缺/坏/非法 → "" 最旧
            install_dv = _zm.valid_data_version(install_zones[zone].get("data_version"))
            return user_dir if state_dv > install_dv else install_dir   # state 严格更新才胜
...
def _read_zone_at(real: Path) -> tuple[list[dict], list[dict]]:
    """从已 resolve 的版本目录读 restaurants+dishes (同代, 不再二次 resolve → 无 torn-pair)."""
    rests = json.loads((real / "restaurants.json").read_text(encoding="utf-8"))
    tagged = json.loads((real / "dishes_tagged.json").read_text(encoding="utf-8"))
    return rests, tagged


def load_zone_data(zone: str, root: Path) -> tuple[list[dict], list[dict]]:
    """读 restaurants + dishes_tagged (同代 realpath 钉读 + ENOENT 兜底回退 install).

    D-102 Step3: 消费前过 manifest 兼容闸门 (引擎↔产物边界, per install_root 只检一次).
    更新机制 (spec §4.6): canonical zone 按 data_version 仲裁; realpath 钉一代后两文件同代读
    (无 torn-pair); symlink 切换/purge race 撞空 → 重 resolve 跟新版本目录, 仍空 → 回退 install.

    兜底边界 (codex Tier-1 review):
    - **只吞 FileNotFoundError** (purge race); JSONDecodeError/PermissionError 等照常上抛 (不掩盖)。
    - Q2: state 版本目录由 install_version 原子写 (temp+rename) 保证完整, 故"缺文件"在正常写路径
      不发生; 退 install = 刻意 degrade-to-known-good-bundle (非静默藏错)。
    - Q5: install 兜底也撞空 → 包成 ZoneNotFoundError (附 state/install 路径), 不冒 raw FileNotFoundError。
    """
    from chisha import manifest
    manifest.ensure_compatible_once(root)
    base = _resolve_zone_dir(zone, root)
    install_dir = root / "data" / zone
    try:
        return _read_zone_at(base.resolve())            # 钉一代读两文件
    except FileNotFoundError:
        try:
            return _read_zone_at(base.resolve())        # symlink 切换/purge race → 跟新版本目录
        except FileNotFoundError:
            try:
                return _read_zone_at(install_dir)       # 仍撞空 → 回退 install (fail-safe)
            except FileNotFoundError as e:
                raise ZoneNotFoundError(                # Q5: install 也缺 → 包装而非 raw FNF
                    f"zone {zone!r} 数据缺失 — state 版本目录消失且 install ({install_dir}) 不完整."
                ) from e
...
def _dish_hits_staple_want(d: dict, staple_want: list[str]) -> bool:
    """D-120: dish 命中点名主食 (canonical+raw 名子串). dish_is_staple 收口防短 token
    (如 "面") 误命中"面包/面筋". staple_want 三处 (dish 加分 / 反查 / combo 桶) 共用."""
    from chisha.score import dish_is_staple
    if not dish_is_staple(d):
        return False
    name = (d.get("canonical_name") or "") + " " + (d.get("raw_name") or "")
    return any(s and s in name for s in staple_want)
...
def _intent_dish_score(d: dict, intent) -> float:
    ...
    # D-120: staple_want (主食诉求, 如 凉面/面/粥) — dish 级加分, 让命中主食浮进 carb 桶
    # (此前 staple_want 全链路未接线, 凉面这类主食菜进不了每店 carb top-3, 被截断扔掉).
    staple_want = getattr(intent, "staple_want", None) or []
    if staple_want and _dish_hits_staple_want(d, staple_want):
        score += 1.0
...
def _ingredient_want_reverse_lookup(
    ...
    ingredient_want = getattr(intent, "ingredient_want", None) or []
    # D-120: staple_want (凉面/面/粥这类主食诉求) 也走反查, 让稀缺主食菜穿透 min_sales 回到候选.
    staple_want = getattr(intent, "staple_want", None) or []
    if not ingredient_want and not staple_want:
        return []
    l0c = load_l0_constraints(profile)
    ...
    for d in dishes_tagged:
        ...
        for ing in ingredient_want:
            ...
            if broad and np_.get("main_ingredient_type") in broad:
                hit = True
                break
        # D-120: staple_want 命中 (主食菜名子串)
        if not hit and staple_want and _dish_hits_staple_want(d, staple_want):
            hit = True
        if hit:
            out.append(d)
    return out
...
def _apply_intent_buckets(
    ...
    *,
    ...
    cuisine_expanded = getattr(intent, "cuisine_candidates_expanded", None) or []
    # D-120: staple_want (凉面/面/粥这类点名主食) 接进桶 + 准硬约束. 此前全链路未消费 → 凉面被稀释.
    staple_want = getattr(intent, "staple_want", None) or []
    ...
    def _combo_has_staple_want(c):
        return any(_dish_hits_staple_want(d, staple_want) for d in c.get("dishes") or [])

    # D-120: ingredient_want / staple_want = 用户点名食材/主食, 准硬约束. 池中命中数 ≥ n →
    # 硬过滤只留命中 combo (宿主无论怎么挑都达标); < n 不过滤 (留兜底) + 命中项进 bucket_soft 置顶.
    # cuisine_want 同存: 先过滤到食材命中再在命中项内按 cuisine 分桶 (两者都满足). 详见 D-120.
    quasi_hard_hit_count = 0
    if ingredient_want or staple_want:
        def _qh_match(c):
            return (any(contains_ingredient(c, ing) for ing in ingredient_want)
                    or _combo_has_staple_want(c))
        qh_hit = [c for c in combos if _qh_match(c)]
        quasi_hard_hit_count = len(qh_hit)
        if quasi_hard_hit_count >= n:
            combos = qh_hit
    ...
    for c in combos:
        if cuisine_want and cuisine_exact_match(c, cuisine_want):
            bucket_exact.append(c)
             ...
             any(contains_ingredient(c, ing) for ing in ingredient_want) or \
             _combo_has_staple_want(c):
    ...
    # D-120 (codex WARN-d): 硬过滤触发后 level 数字仍按 intent_hit 三档算, 但语义不再是
    # "稀缺返全集" — event 的 quasi_hard_filtered (= 命中≥n) 让消费方区分"硬过滤成功" vs "稀缺兜底".
    ...
    if recall_fallback_events is not None:
        ...
        recall_fallback_events.append({
            ...
            # D-120: 准硬过滤是否触发 (命中≥n → 池只留命中) + 命中总数. 区分硬过滤成功 vs 稀缺兜底.
            "quasi_hard_filtered": quasi_hard_hit_count >= n,
            "quasi_hard_hit_count": quasi_hard_hit_count,
