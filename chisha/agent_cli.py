...
do_llm 信封 (canonical wire key), agent 的 LLM 执行后按 correlation_id 回传, chisha 校验落库.
...
verb 链 (P1 折叠后, 顶层 `chisha eat/continue/choose` 见 cli.py):
  ...
  choose   --id <rid> (--card <cid> | --rank <N>) --action <accept|skip>   记录选择 (幂等; --rank=cron 续轮按号定位)
...
def _resolve_profile_zone(meal_type: str, root: Path,
                          override_zone: str | None = None) -> tuple[dict, str]:
    """轻量解析 (profile, zone), 不加载 zone 数据 — M3.2 region guard 用 (unsupported zone
    不该因 load_zone_data 抛错). 是 _load_inputs 的前半段, 抽出作可 mock 的 seam。

    override_zone (--zone 临时覆盖, 出差/换区, **不改 profile 默认**) 优先于 profile;
    仍可能是未归一的 slug (如 "景湖"), canonical 化在 cmd_start 经 resolve_region 完成。"""
    ...
    zone = override_zone or _resolve_zone(profile, meal_type)
    return profile, zone


def _load_inputs(meal_type: str, root: Path,
                 zone: str | None = None) -> tuple[dict, str, list, list, list]:
    from chisha.recall import load_meal_log, load_zone_data
    profile, resolved = _resolve_profile_zone(meal_type, root, override_zone=zone)
    rests, tagged = load_zone_data(resolved, root)
    meal_log = load_meal_log(root)
    return profile, resolved, rests, tagged, meal_log


def _inherited_zone(sid: str, root: Path) -> str | None:
    """refine `--from` 未显式 `--zone` 时, 继承上一轮**实际服务的** zone (而非 profile 默认),
    让出差续轮留在同一区。round_store 在 apply 成功后已 clear、trace 发布是 best-effort 可能
    缺 → 用 ready 快照 (每轮 apply 必可靠写、跨轮存活, 与 cards __latest 同权威级) 作继承源。
    canonical zone_id 由 cmd_start 写 ready 时存入 __zone。缺 → None (回退 profile)。"""
    latest = _latest_published_round(sid, root)
    if not latest:
        return None
    return (_read_ready(sid, latest, root) or {}).get("__zone")
...
def _find_card_in_latest(sid: str, root: Path, match) -> tuple[dict | None, str | None]:
    """choose: 在 cards 文件**最新一轮**找首个 `match(card)` 为真的卡 (codex #c: 只查
    __latest 轮, 不跨轮 — 用户永远从当前呈现里选; card_id/rank 无 round 成分跨轮可重名).

    `_find_card`(按 id) 与 `_find_card_by_rank`(按 rank, cron 续轮) 共用此单源, 保
    「只查最新轮」语义不在两处漂移。返回 (card, round_id) 或 (None, latest_round)。
    """
    data = _read_round_keyed_json(_cards_path(sid, root))
...
def _find_card(sid: str, card_id: str, root: Path) -> tuple[dict | None, str | None]:
    """按 card_id 在最新一轮找卡 (用户从当前呈现里选)。"""
    return _find_card_in_latest(sid, root, lambda c: c.get("id") == card_id)


def _find_card_by_rank(sid: str, rank: int, root: Path) -> tuple[dict | None, str | None]:
    """D-121 M4: cron 续轮按 rank 号定位卡 —— cron isolated turn 发完纯文本提醒就结束,
    用户回"选 1"是新 turn, 没有 card.id 在手, 只有提醒里的 recommendation_id + 号。"""
    return _find_card_in_latest(sid, root, lambda c: c.get("rank") == rank)
...
def _write_ready(sid: str, round_id: str, ready: dict, step_token: str,
                 root: Path, zone: str | None = None) -> None:
    """P1 (codex Q3): apply 成功落 ready 响应快照 (含 step_token), 供 continue rerank
    重发幂等回放 (round clear 后宿主分不清"完成"vs"丢失"). round 维度累加.

    zone_bug_fix: 内部键 `__zone` 存本轮 canonical zone (供 refine `--from` 继承), 不进
    对外回包 — 回放时 continue 须连 step_token 一起 strip。"""
    _append_round_keyed_json(_ready_path(sid, root), round_id,
                             {**ready, "step_token": step_token, "__zone": zone})
...
def _prepare_and_rerank_spec(
    *, sid: str, round_id: str, meal_type: str, today: dt.date,
    ...
    zone = frozen 冻结的 canonical zone (override/继承/profile 三选一), None 时退 profile。"""
    from chisha.agent_orchestration import prepare_candidates
    from chisha.rerank import build_rerank_spec
    profile, zone, rests, tagged, meal_log = _load_inputs(meal_type, root, zone=zone)
...
def cmd_start(args) -> int:
    ...
    # M3.2 + zone override (zone_bug_fix): region 状态闸门守的是**请求 zone**。
    # zone 解析优先级 = `--zone` 临时覆盖 (出差/换区, 不改 profile) > refine `--from` 继承上轮
    # 服务 zone > profile 默认。unsupported/coming_soon → coming_soon 终态 (不进 recall/score、
    # 不建 round — codex G3); 这堵住了"出差到未覆盖区时误用 profile zone 数据复述成异地推荐"。
    # 归一 canonical zone_id (override 可能传 slug "景湖"→jinghu) 经 resolve_region。
    # 早于 round 持久化, context/no-context 共守。G2: profile 缺失/manifest 损坏 = 真错。
    from chisha.core_api_helpers import (
        build_coming_soon_response, resolve_region)
    override_zone = getattr(args, "zone", None)
    if override_zone is None and args.from_id:
        override_zone = _inherited_zone(args.from_id, root)
    try:
        _guard_profile, _requested_zone = _resolve_profile_zone(
            args.meal, root, override_zone=override_zone)
        _region = resolve_region(_requested_zone, root)
    except Exception as e:
        return _emit_error("LOAD_INPUTS", f"{type(e).__name__}: {e}")
    # canonical zone_id (slug 命中时归一 "景湖"→jinghu; unversioned/unsupported 无 manifest
    # 项时退请求原值). coming_soon 终态回包与 eatable 冻结统一用 canonical (codex diff review #1)。
    zone = (_region.get("zone") or {}).get("zone_id") or _requested_zone
    if not _region["eatable"]:
        _emit(build_coming_soon_response(zone, _region, _guard_profile))
        return 0
    ...
    if context:
        ...
        try:
            _load_inputs(args.meal, root, zone=zone)  # 早失败: zone 数据缺失立刻报
        except Exception as e:
            return _emit_error("LOAD_INPUTS", f"{type(e).__name__}: {e}")
        ...
        frozen = {
            "meal_type": args.meal, "zone": zone, "today": today.isoformat(),
        ...
        _emit({
            ...
            "do_llm": extract_spec,            # canonical (host 判 do_llm 在否驱动循环)
            "step_token": cid.encode(),        # continue 回显此 token (不透明)
    ...
    _emit({
        ...
        "do_llm": spec,                    # canonical
        "step_token": cid.encode(),        # continue 回显此 token
        "next": "continue --id <rid> --result <json> --step <step_token>",
    })
    return 0
...
def cmd_resolve_intent(args) -> int:
    ...
    if cur.get("status") == "resolved" and cur.get("extract_spec") is not None:
        _emit({
            ...
            "do_llm": cur.get("rerank_spec"),            # canonical
    ...
    try:
        prep, spec = _prepare_and_rerank_spec(
            ...
            fb_signal=fb_signal, root=root, zone=frozen.get("zone"),
        )
    except Exception as e:
        return _emit_error("PREPARE", f"{type(e).__name__}: {e}")
...
def cmd_apply_rerank(args) -> int:
    ...
    # M3.3: 派生 profile_status + profile_notice 注入 ready (节 3c). best-effort
    # (失败不阻断 cards). 写进 ready dict → _write_ready 快照 → continue 幂等回放自带 (codex G1).
    profile_status, profile_notice, _ap_profile = None, None, None
    try:
        from chisha.core_api_helpers import (
            build_profile_notice, build_profile_status, resolve_region)
        # zone_bug_fix: ready banner 的 region 必须反映**本轮服务的** zone (frozen.zone,
        # 出差时是景湖大厦而非 profile 的深圳湾), 不再重读 profile。旧 round 无 frozen.zone
        # → override_zone=None → 退 profile (向后兼容)。
        _ap_profile, _ap_zone = _resolve_profile_zone(
            frozen["meal_type"], root, override_zone=frozen.get("zone"))
        profile_status = build_profile_status(
            _ap_profile, _ap_zone, resolve_region(_ap_zone, root))
        # D-112 Step1: sid 透传 → once_key (session=sid, 跨 refine 轮去重 once_per_session)。
        profile_notice = build_profile_notice(profile_status, sid=args.id)
    except Exception as e:
        # best-effort: 不阻断 cards (用户拿到推荐优先于画像状态展示), 但**不静默** —
        # log 出来防真 bug 被吞 (codex Q(b)). cmd_start guard 已保证 zone available, 此处
        # 正常不该失败; 失败多半是 manifest 在 start→apply 间被删等边角。
        import logging
        logging.getLogger(__name__).warning(
            "profile_status 派生失败 (cards 不受影响): %s: %s", type(e).__name__, e)
    ...
    ready = {
        ...
        "narrative": narrative,
        "cards": cards,
        "profile_status": profile_status,   # M3.3 (节 3c): region/taste/methodology 状态
        "profile_notice": profile_notice,   # M3.3: taste 空时的软性补全邀请, 否则 None
    }
    # 更新机制 P3: 数据陈旧度提示 (SKILL.md 指示宿主在推荐末尾附一行; 离线/失败静默, 绝不阻塞).
    try:
        from chisha import staleness as _stale
        _st = _stale.compute_for_root(root)
        if _st:
            ready["staleness_notice"] = _st
    except Exception:
        pass
    # 更新机制 P4: profile 问卷版本过期提示 (best-effort, 不阻塞). 用本函数已加载的 _ap_profile。
    try:
        from chisha import agent_profile_setup as _aps
        if _ap_profile and _aps.profile_schema_outdated(_ap_profile):
            ready["profile_schema_notice"] = {
                "current": _aps.QUESTION_SCHEMA_VERSION,
                "message": "onboard 问卷已更新, 可跑 `chisha onboard --resume` 补答新增项",
            }
    except Exception:
        pass
    # P1 (codex Q3): 落 ready 快照供 continue rerank 重发幂等回放 (round clear 前).
    # zone_bug_fix: 快照内附 canonical zone, 供 refine `--from` 跨轮继承 (_inherited_zone)。
...
def _publish_trace_best_effort(*, sid, round_id, frozen, persisted, mapped, today,
    ...
    try:
        ...
        profile, zone, rests, tagged, meal_log = _load_inputs(
            frozen["meal_type"], root, zone=frozen.get("zone"))
    ...
    if tok.operation == "rerank":
        ...
        if cur is None:
            ...
            if (ready is not None and ready.get("step_token") == args.step
                ...
                out = {k: v for k, v in ready.items()
                       if k not in ("step_token", "__zone")}
                out["replayed"] = True
...
def cmd_choose(args) -> int:
    ...
    # D-121 M4: --card <cid> 与 --rank <N> 二选一 (后者为 cron 续轮按号定位)。
    card_arg = getattr(args, "card", None)
    rank_arg = getattr(args, "rank", None)
    if (card_arg is None) == (rank_arg is None):
        return _emit_error(
            "BAD_ARGS",
            "choose 须 --card <cid> 或 --rank <N> 二选一 (cron 续轮用 --rank)",
        )

    try:
        _validate_id(args.id, "--id")
        if card_arg is not None:
            _validate_id(card_arg, "--card")
    except RuntimeError as e:
    ...
    if rank_arg is not None:
        # M4: 按 rank 号解析回 card.id (rank 越界即报错, 不静默乱选)。
        card, round_id = _find_card_by_rank(args.id, rank_arg, root)
        if card is None:
            return _emit_error(
                "RANK_NOT_FOUND",
                f"rank {rank_arg} 不在 {args.id} 最新一轮 cards 里 (cron 续轮: 核对提醒里的号)",
            )
        card_id = card["id"]
    else:
        card_id = card_arg
        card, round_id = _find_card(args.id, card_id, root)
        if card is None and args.action == "accept":
            return _emit_error(
                "CARD_NOT_FOUND",
                f"card {card_id!r} 不在 {args.id} 最新一轮的 final cards 里 (accept 需卡片明细)",
            )
    rest = (card or {}).get("restaurant") or {}
    dishes = (card or {}).get("dishes") or []
    out = agent_choose.record_choice(
        root, sid=args.id, card_id=card_id, action=args.action,
...
def cmd_doctor(args) -> int:
    ...
    # 更新机制 P3: 数据陈旧度提示 (notify-don't-force, 离线静默, 绝不改 ok / 不阻塞)
    from chisha import staleness as _stale
    _st = _stale.compute_for_root(install_root)
    if _st:
        info["notes"].append(_st["message"])
        info["staleness"] = _st
    # 更新机制 P4: profile 问卷版本过期提示 (best-effort, 不阻塞 / 不改 ok)
    try:
        import yaml as _yaml
        from chisha import agent_profile_setup as _aps, data_root as _dr
        _ppath = _dr.profile_path(root)
        if _ppath.exists():
            _prof = _yaml.safe_load(_ppath.read_text(encoding="utf-8")) or {}
            if _aps.profile_schema_outdated(_prof):
                info["profile_schema_outdated"] = True
                info["notes"].append(
                    f"onboard 问卷已更新 (当前 {_aps.QUESTION_SCHEMA_VERSION}), "
                    "建议 `chisha onboard --resume` 补答新增项")
    except Exception:
        pass
    _emit(info)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--id", required=True)
    sp.add_argument("--card", default=None, help="card_id (apply-rerank 返回的 cards[].id)")
