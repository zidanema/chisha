...
def api_history(days: int = 7) -> dict:
    ...
    # D-074 Codex S3 修复: 走 data_root.recommend_log_path, sandbox 启用时
...
# ---------- /api/long_term_prefs/refresh (D-073 PR-0.9) ----------
...
def api_long_term_prefs_refresh(request: Request, req: RefreshPrefsReq) -> dict:
    """D-073 PR-0.9: 手动触发 L1 LLM 抽取, 写入 data/long_term_prefs.json.
...
# D-074 Codex S3 修复: L1 抽取串行锁, 防多 tab 并发 advance 同时跑多个 LLM 抽取
...
def _trigger_l1_extraction_async(root: Path) -> None:
    ...
    并发保护 (D-074 Codex S3): 用 trylock 模式 — 已有 worker 跑时直接跳过,
