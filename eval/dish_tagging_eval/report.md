...
| 数据量 1万-10万 / 准确率可放宽 | **`haiku-4.5`** | 字段准确率 85.2%(距冠军 -4.2pp), 但吞吐 4.4 条/秒, 10万条 7.9h, $142 |
| 数据量 ≥ 10万 / 时间不敏感 | **`deepseek-flash`** | 100万条仅 $100 (冠军的 2.2%), 但 10万条要 19.4h, 准确率 88.9% |

### 4. 是否分层方案?

**建议分层**:`haiku-4.5` 主跑 + `sonnet-4.6` 回退低置信样本.
- 主路径:`haiku-4.5` 跑生产数据冲刷, 准确率 85.2%, 跑 10万条 $142 / 7.9h
- 回退路径:对 JSON 不合法 / 关键字段反直觉(`sweet_sauce_level<2` 但触发词命中, `processed_meat_flag=true` 但烧腊命名 等)的样本, 用 `sonnet-4.6` 重跑
- 准确率 gap 4.2pp 通过分层可拉平到接近 `sonnet-4.6` 水平

### 5. 最终决策 (基于本次实测)

- 如果**只跑一次几千条 → `sonnet-4.6`**:准确率 89.4% + 速度最快档 + 一次性成本可接受
- 如果**几万条且要在 1 天内出结果 → `haiku-4.5`**:吞吐最快, 1 万条 $14 / 10 万条 $142, 准确率 85.2% 可被分层补救
- 如果**几十万条 + 离线异步 + 极致省钱 → `deepseek-flash`**:成本几乎可忽略, 但要忍受 19.4h/10万条的耗时
