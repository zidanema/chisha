...
- **派生 feedback 信号必须可观测 (D-083)。** 写 `score.score_breakdown` 的派生维度 (`feedback_recency` / `next_meal_calibration` / `note_boost`) 必须同时在 trace `feedback_view_snapshot` 留**输入快照** (per-entry signal/decay/triggers) + per-combo `feedback_evidence` (sibling, **不入 breakdown** 防破 `_format_ranked_for_trace` 的 `round(v,3)` numeric 契约)。L3 prompt 渲染段同步 `l3.feedback_block_rendered` 单字段，别只埋 `user_message_full`。What-if `__frozen.feedback_view` 已经存了完整 dict — 加新字段只动 `build_feedback_view._build_feedback_trace_snapshot` 即可。L1 prefs 派生层暂留 D-083 范围外，后续单独决策号开。
