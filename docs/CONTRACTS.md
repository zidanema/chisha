...
- **所有 user state 路径经 `state_root.resolve(root)` 解析 (单一权威源), 绝不裸拼 `root/"logs"...`.** data_root 9 落盘函数 + sandbox 路径基底 (`_state_base`) + web_api sandbox 路由 (`_sb_bucket`/`_validate_and_route_sid`/桶校验/`_copy_real_data`) + sandbox_migration 全收口经它, 否则 enable 写一处读另一处 split-brain。新增写 state 的路径加进来, 别再开第二条解析。
