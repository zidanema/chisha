...
> **⚠️ 反馈闭环 P3 部分已被 [D-073](#d-073-l1-长期反馈层重构--砍伪-l1--llm-抽取-v1x) superseded (2026-05-16)**:
> "refine chip → feedback_history.jsonl 频次聚合 → load_runtime_hints" 这条路径
> 概念错位 (refine chip 是 L2 单次信号, 不该跨 session). D-073 砍掉此路径,
...
## D-073: L1 长期反馈层重构 — 砍伪 L1 + LLM 抽取 (V1.x)
...
- 关联: D-074 sandbox 模式提供真实场景验证 L1 抽取链路
...
## D-074: Sandbox Time-Travel 模式 (V1.x)
...
不重店 / 3d 不重蛋白 / snooze 24h / session ttl 24h / D-073 新 L1
...
D-编号占用:
- D-074 此前 memory 占位 "AI-friendly 接入共识" (chisha_ai_friendly_
  consensus_d074.md). 该草稿未落 DECISIONS.md, 实际编号未发, 让位
  给 sandbox; AI-friendly 真正落条目时改用 D-075+.

依赖 / 影响:
- 配套 D-073 L1 LLM 抽取层 (sandbox 验证它的核心场景)
...
## D-075: Sandbox 时钟漏注入修补 + accept→meal_log 闭环 cooldown (V1.x)

日期: 2026-05-16
状态: active · 落地: 2026-05-16 (1 commit, S2 Codex review 两轮)

背景:
D-074 sandbox 落地后用户视角 e2e 自验 (5 日推进真实 LLM 闭环) 抓到 3 个连带
...
1. **L1 时钟漏注入 (P0)**: `l1_extractor.aggregate_inputs` 默认 today 用
   ...
   空. D-074 PR-1a 自称「11 处时间注入」, 漏了第 12 处.
2. **meal_log 写入端缺失 (P1)**: D-074 文档自认 V1 gap, 但 ROADMAP 又说
...
D-编号占用:
- 检查 docs/DECISIONS.md 末尾确认 D-075 空缺 (memory 草稿 ai_friendly 没真正
  落 DECISIONS, 按"先到先得"占 D-075). AI-friendly 真正落条目改用 D-076+.

不在 D-075 内:
- diversity_filter 按 zone 过滤 (跨 zone 污染概率低, 留 D-076+ 决策)
- reset/disable 抢锁失败的完整 dirty-flag 补跑机制 (D-075.1 候补)
...
依赖 / 影响:
- 修补 D-073 (L1 LLM 抽取层) + D-074 (Sandbox Time-Travel) 落地后的连带空洞
