...
- **未落地 (待周末重采居住区)**: A3 `observed_address_text` 非 null 需真机验; B3 全链路 (tag/validate) + 跨 zone 哨兵需重采两 zone 真数据才能真跑 (home 现已 purge, 单 zone 哨兵自动跳过). **office 真文件 (`~/waimai_data/output/`) 2026-05-26 实测仍 pre-D-100 schema (collector Batch A 未进日常输出)** → 上文「真 A4 office 文件过契约」守门暂失效 (loader 契约对它会 fail); 真文件契约测试改「缺 schema_version 即 skip」, collector 重导出带 schema_version 的 envelope 后自动恢复把关.
