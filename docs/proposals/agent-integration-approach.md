...
> **PROPOSAL · 未落地**. D-074 草稿配套技术交流文章, 不是已实现的接入方案。
> V1.0 工程收尾后, 这套接入方案仍是 Phase 1 推广启动后的目标终态; D-074 翻 active 时本文回到 `docs/` 顶层并同步实现细节。
> 当前已实现的接入接口见 [../api.md](../api.md)。
...
**飞书 CLI** 是某大厂内部开放给 AI 操作飞书产品的成熟实践 (一个 `lark-cli` 命令行 + 一组 `lark-*` skill markdown 文件, 覆盖 IM / 邮件 / 多维表格 / 文档 / 日历 等近 20 个领域), 验证了这个模式适合**大公司多产品对接 Agent**。chisha 是同一模式的个人产品级应用, 验证了它也适合**个人垂直能力对接 Agent**。
