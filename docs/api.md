...
> 适用于 `apps/web` ↔ `chisha/debug_server.py` (端口 8765, D-051)。
...
| `POST` | `/api/skip` | 跳过这餐 → 出 acceptedQueue, banner 不再弹 (D-054) | V1 ✅ |
| `GET`  | `/api/feedback/inbox?include_snoozed=` | 反馈中心列表 (代替 last_unfed 单条, **D-058**) | V1.1 ✅ mock |
| `POST` | `/api/feedback/snooze` | 24h 软关闭, banner 不弹但 inbox 仍在 (**D-060**) | V1.1 ✅ mock |
| `POST` | `/api/feedback/stop`   | 永久硬关闭, banner+inbox 都隐藏 (**D-060**) | V1.1 ✅ mock |
...
| `GET`  | `/api/feedback/<session_id>/record` | 已提交反馈记录 (form/detail 双态判断, **D-066**) | V1.1 ✅ mock |
| `POST` | `/api/feedback` | 提交反馈 (V1.1 schema, **D-063~D-065**) | V1.1 ✅ mock |
| `POST` | `/api/feedback/<session_id>/comments` | append-only 追加备注 (**D-067**) | V1.1 ✅ mock |
...
> **不假装成功**: 前端**不依赖**这个 deeplink_url 真能拉起 APP (iOS/Android 拉起率 < 30%, D-052)。前端总是显示"搜店名 + 复制按钮"。`deeplink_url` 仅作为最佳努力 fallback。

### 6.4 `POST /api/skip` (D-054)
...
- `reason` 进推荐学习信号 (在外吃/食堂 ≠ 都没看上, 见 D-054)

### 6.5 反馈端点 (D-056~D-068, V1.1)
...
- D-055 同 session 抑制保留: 主页 banner 过滤掉当前 home session_id

**`POST /api/feedback/snooze`** body `{ session_id }` — D-060 软关闭
- 后端给 acceptedQueue 项设 `snoozed_until = now + 24h`
- inbox `snoozed` 字段在 24h 内为 true, 之后自动回 false (无需用户操作)
- 不进推荐学习信号

**`POST /api/feedback/stop`** body `{ session_id }` — D-060 硬关闭
- 后端给 acceptedQueue 项设 `stopped = true` (永久)
- inbox 列表过滤掉 stopped 项 (banner + inbox 都不显示)
- 不影响 history (D-059 history 行仍可点)
...
**`GET /api/feedback/<session_id>/record`** — getFeedback (D-066 双态判断)
...
**`POST /api/feedback`** — 提交反馈 (D-066 提交即 readonly)
...
- `reason_match / fullness / oil_calibration / repurchase_intent: 0 | 1 | 2 | null` — 4 维 calibration/behavior (D-065), `null` 表示用户没填
...
- 同 session_id 提交多次 = 覆盖 payload 但 **comments[] 保留** (D-067 隔离原始 + append)
- D-066 spec 上**禁止编辑**, 但当前 mock 没强制 (后端实施时建议 4xx if exists, 由前端 detail view UI 保证不发 duplicate)
- inbox 该 session 立即从「待反馈」段消失

**`POST /api/feedback/<session_id>/comments`** body `{ text }` — D-067 append-only

副作用: push 新 comment 到 `feedbacks[sid].comments`, 每条独立 timestamped, 不修改原始反馈字段。返回 `{ ok: true }`。

> **不删除 / 不编辑 (V1.1)**: 没有 `DELETE /api/feedback/<sid>` 或 `PATCH /api/feedback/<sid>/comments/<cmt_id>` 端点 (D-066 + D-068), V2 再加。
...
- V2.0 反馈系统增量 (D-068): 删除反馈 / 撤销重提 / comment 结构化 chip / banner 一键打分 / A/B 实验框架
- 反馈数据回灌推荐推理: `reason_match` reverse-loss 给 LLM reason generator / `repurchase_intent` 进 ranking / `comments[]` 给同店推荐 reason 生成时 context inject (D-063~D-065, 后端实施待装)
