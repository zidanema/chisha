...
- **状态**: **部分完成** (2026-05-30 重核+分批落地, 见 git log "F-016")。已落: 安全批 (scripts/loader 长函数+去重) + 白名单批 refine/_resolve_zone 单源/status_bar 统一/agent_cli 去重+拆/manifest 工厂/feedback_signal 抽取 (各过 baseline_l2 0-diff + codex review) + 前端 16 项结构性重构 (web/debug-ui/sandbox-lab, 各过 tsc typecheck)。chrome-devtools 浏览器自测覆盖: debug-ui trace 列表+详情完整渲染 (L1→L3 pipeline/recall 漏斗/dim_stats, 网络全 200 零 console error) ✅; web HomePage 推荐 + ProfilePage 编辑态 7 组渲染 ✅; web feedback 详情 golden path (表单→提交→FeedbackDetailView→D-067 追加, 全 200) ✅ (2026-05-31 补串); sandbox-lab live eat (POST /sandbox/init 后真后端 eat→job→snapshot 全 200, 白盒状态机推进) ✅ (2026-05-31 补串)。补串时定位+修一个独立 bug: feedback 详情 session 缺 payload → GET /api/feedback/{id} 404 → Promise.all reject 永久卡 loading (空白), 改 allSettled 兜底错误卡 (非 F-016 重构回归, 见 git "修复: web feedback 详情")。重核剔除 2 项已死 (审计快照 stale)。
...
- **2026-05-31 第二批落地** (各过 baseline_l2 0-diff + 守门 pytest + codex 共商 design + diff review): ③ `l1_extractor.aggregate_inputs` 拆 orchestrator + 5 helper ✅; ④ `recall.build_combos_for_restaurant` 拆 orchestrator + 4 helper + 加锁输出顺序 test ✅; ⑥ `dim_stats` 四处逐字重复收敛 `trace_helpers.dim_stats_topk` ✅; ② `trace_store` v2-meta 去重抽 `_v2_base_meta` (list_traces + list_traces_v3 v2 分支; **不并入** `_build_meta_from_v2` — 它产持久化 meta key=`__source` 与 API key=`source` 是两 schema, codex 共商) ✅; ① `agent_cli.cmd_doctor` 抽 `_doctor_notes` (notes 文案与检查逻辑分离) ✅。⑤ `atomic_io` 早前已落 (本批前; BACKLOG 原"剩余"对 ⑤ 描述 stale)。
- **剩余 defer (后续单独做)**: ① 余两长函数 — `web_api._rollback_session_impl` (~243行, 精细事务函数, 拆 helper 净负 + 仅 pytest 无 baseline, codex/志丹判 defer) / `rerank._run_llm_rerank` (~337行, **动 L3 质量, baseline 守不住 + 无自动守门** → 留独立会话 + 人工 dry_run 验)。两者均碰 high-risk 白名单 (web_api/rerank), 真做时 codex 共商 + pytest 守门。
