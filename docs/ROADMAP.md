...
**Phase 0 工程侧已收尾**（2026-05-18，D-001~D-085）—— 推荐链路 L1/L2/L3 全跑通 + Web SPA + V1.1 反馈 + L1 真兑现（LLM 抽取）+ Sandbox Time-Travel + trace 持久化 + Debug 三模式（Replay / What-if / Live）+ FastAPI 23 端点 + **Refine v2 / Faithful Refine framework 重构**（D-080~D-085: L0 三分 + RefineIntentV2 多 slot + reference resolver + subtype diversify + always-on 方法论状态条 + L3 narrative 透传, 见 brief）。L1 → L2 → L3 三层贯通,真实 LLM 演练验证通过。

历史细节(Phase 0 全量 commit-level 决策) 在 [archive/DECISIONS_phase0.md](archive/DECISIONS_phase0.md);活决策在 [decisions.md](decisions.md);跨文件约束在 [CONTRACTS.md](CONTRACTS.md)。

## Phase 0 收尾路线（2026-05-17 拍板）

终极路径:**自用稳定 → 接个人 agent(含 context 注入) → 扩同事 / 数据源**。基于此反推最近 4-6 周做事顺序:

1. **Debug trace 串接验收** (1-2 天) — claude code 自测过但未验收。所有后续动作的归因基础设施,先把它跑可信
2. **摸清 L1 抽取 prompt + 链路** (半天-1 天) — 不学整套系统,只看清 L1 这层
3. **沙盒 e2e 跑通 + D-080~D-085 Refine v2 framework 复测** (2-3 天) — 沙盒主战场,真实日历日并行后台跑,不强门。设计 brief: [`design_briefs/2026-05-18-refine-v2-faithful-refine.md`](design_briefs/2026-05-18-refine-v2-faithful-refine.md)
4. **接个人 agent + context 注入** (捆绑,不拆) — 这步是放大 chisha 价值的拐点。agent 注入"素食 / 生病 / 临时偏好"自然语言意图 → chisha 当 refine 走 L1→L2→L3
5. **同 query 引入可控随机性** — 接 agent 后频次拉高才有体感场景
...
### Phase 1 推广启动前必收口(不在当前周次,但是硬门)

- 第二份 methodology spec (验证抽象解耦)
- screener 设计 (筛同事)
- 沙箱模式交互动线重设计 (推广前用户心智不能乱)
- L1 词表扩 cuisine token (同事 cuisine 比志丹分散)
- B-001 v2 (反馈短链路全字段覆盖, 见 [`wip/B-001-v2-design-brief.md`](wip/B-001-v2-design-brief.md))
- Living/Lab router 拆分重做 (回滚后待重做)
...
Phase 0 · 自用跑通 (当前, 工程侧已收尾, 等用户行为验证)
  ...
  步骤:
    ...
    Step 2 [进行中, 工程+用户并行]: 沙盒 e2e + 真实日历日并行 (见上方"Phase 0 收尾路线" 5 步)
  ...
  门槛: 自己愿意每天用 + 接个人 agent 跑通
        ↓ 验证: 沙盒 D-080/D-081 复测 OK + 真实日历日不抗拒
Phase 1 · 同方法论同事推广
...
- [x] ~~**macOS 本机定时拉起服务**~~ → **2026-05-17 划掉**：自用阶段手动开页面没毛病, 不是债 (志丹拍板)
...
- [ ] **Step 2：沙盒 e2e + 真实日历并行** (2026-05-17 修订): 改成沙盒主战场 + 真实日历后台跑, 不再设"工作日 7 日采纳率 ≥ 50%" 强门槛。详见顶部 "Phase 0 收尾路线" 5 步。
...
| 自用稳定性 | ~~工作日 7 日采纳率 ≥ 50%~~ → **2026-05-17 修订**: 沙盒 D-080/D-081 复测 OK + 真实日历日不抗拒 + 接个人 agent 跑通 | 🔄 沙盒侧推进中 |
...
> **触发条件**：自用稳定 (沙盒 D-080/D-081 复测 OK + 真实日历日不抗拒) + 个人 agent 接入跑通, 主线推荐链路稳定。
