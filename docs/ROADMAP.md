...
**Phase 0 工程侧已收尾**（2026-05-17，D-001~D-079）—— 「原则派点餐执行外包」定位收敛 + L0 方法论 spec 抽象 + **L1 长期反馈层真兑现（LLM 抽取）** + **Sandbox Time-Travel 模式** + **推荐链路 trace 持久化 + Debug 三模式（Replay / What-if / Live）** + 推荐 L1/L2/L3 全跑通 + Web SPA + V1.1 反馈 + FastAPI 23 端点（推荐 6 + V1.1 反馈 7 + L1 刷新 1 + sandbox 6 + trace 3）。L1 → L2 → L3 三层贯通，真实 LLM 演练验证（low_oil + spicy boost）。Step 2 自用走 sandbox 压缩到分钟级，不必等真实日历日 → Phase 1 同事推广。
...
- **并行启动：Refine v2 / Faithful Refine 推荐链路重设计**（D-080~D-085，2026-05-18，design brief: [`docs/design_briefs/2026-05-18-refine-v2-faithful-refine.md`](design_briefs/2026-05-18-refine-v2-faithful-refine.md)）：志丹自用过程暴露两个体感问题（refine 自然语言诉求没被很好理解和满足 + trace 送 L3 的 combo 凑不满 60）。Opus + Codex 两轮 review 后收敛设计。核心：①Faithful Refine 第一原则进 CONTRACTS（D-080）②refine 多 slot LLM 解析（D-081）③L0 三分判定表替代旧"硬契约 vs 破戒"二分（D-082）④methodology 硬契约下沉 L1 修最大产品 bug（D-083）⑤refine 模式 L1 召回参数走差异化分支（D-084）⑥narrative + 状态条必须后置避免信任放大器（D-085）。实施次序：前置 → P1a 三模块并行 → P1b 输出层（依赖 P1a） → P2 reference 块 + 簇式输出。任务清单见 brief §10。

---

## Phase 路线（[D-070](archive/DECISIONS_phase0.md#d-070-产品定位收敛到原则派点餐助手--三层信号模型-v1) 沉淀，取代旧 V1/V2/V3 笛卡尔积）
...
- [x] **v3 全量重打 + dual-model golden 171 条**（[D-032](archive/DECISIONS_phase0.md#d-032) / [D-036](archive/DECISIONS_phase0.md#d-036)，2026-05-12）：13,240 菜 + 4 大字段一致率 99.27%
- [x] **6 模型横评 + 生产默认模型确认**（[D-037](archive/DECISIONS_phase0.md#d-037)）：deepseek-v4-flash 字段 acc 88.9%，100万条 $100
...
- learned_profile 统计聚合 → V2.2（不再做 LLM 蒸馏 insights，[D-026](archive/DECISIONS_phase0.md#d-026)）
- LLM 抽象 Phase 2 callable 注入点 → **方案 2026-05-16 已被 D-074 共识改方向**（[D-038](archive/DECISIONS_phase0.md#d-038) closure 注入 → `llm_request_spec` machine-readable 数据契约，详见 [design_briefs/2026-05-16-ai-friendly-integration-v2-consensus.md](design_briefs/2026-05-16-ai-friendly-integration-v2-consensus.md)）
...
> 目标：把"采集 / 清洗打标 / 对外数据服务"三件事拆成单仓内的独立子模块（[D-030](archive/DECISIONS_phase0.md#d-030)）。
...
- [ ] meal_log.jsonl 完整记录（dish 字段补 cuisine + cooking_method，[D-025](archive/DECISIONS_phase0.md#d-025)）
...
- [ ] **个性化 refine 快捷标签**（来自 [D-051](archive/DECISIONS_phase0.md#d-051) Web 用户视图设计 review）：当前 V1 用 6 个静态标签（想吃辣的 / 换日料 / 来份烧烤 / 想吃牛肉 / 来盖饭 / 换粤菜，见 `docs/design_briefs/v1_user_view.md` §5.4.1）；V2.1 改为根据"最近 3 天没吃过的菜系/食材 + learned_profile bottom_preferences 反向 + 当前 mood"动态生成 4-6 个标签。新增 API: `GET /api/recommend` 响应里加 `suggested_refine_tags: string[]` 字段
...
- [ ] 加工脚本（每周日凌晨自动跑）
- [ ] 数据加权策略（D-013）
- [ ] **统计聚合到 (cuisine, cooking_method, main_ingredient) 维度**（[D-026](archive/DECISIONS_phase0.md#d-026)）
...
- [ ] L1 数据按工区拆 `chisha-data-shenzhen-keji` / `chisha-data-beijing-zgc` 等子包（[D-002](archive/DECISIONS_phase0.md#d-002) 修订）
- [ ] 包发布到 PyPI（先内部，再公开）
- [ ] 订阅 `chisha-collector` 的 15 天更新机制（[D-027](archive/DECISIONS_phase0.md#d-027)）
