...
>
> **D-102 (2026-05-28) 重激活 Phase 1 工程主线 = 可分发共享核心** (不推翻 D-097 自用心态, 把"真要推广时恢复"那一刻落地). 分步: Step1 焊大脑 (FallbackPlan, **已落地 D-102.1**) → Step2 root 拆分 →`~/.chisha/` (**已落地 D-102.2**, install/state 二分 + 迁移 + doctor) → Step3 manifest + capability-flag compat 闸门 (**已落地 D-102.3**; integrity 签名/plugin marketplace 留位, 先内部 git transport). **D-102 三步落地完毕**. 提案 `docs/proposals/2026-05-28-distributable-shared-core.md`.
