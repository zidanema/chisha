...
def test_default_llm_call_uses_existing_llm_client_symbol(monkeypatch):
    """D-078 回归守门: _default_llm_call 必须 import 真实存在的符号.
...
def test_aggregate_default_today_uses_chisha_clock(monkeypatch, tmp_path):
    """D-078 修补: aggregate_inputs(today=None) 必须走 chisha.clock.today(root=root),
    ...
    assert summary["based_on_meals"] == 1, (
        ...
        "(D-078 bug 复现: 若回归到 dt.date.today() 则 ts=2026-05-30 > today=2026-05-16 被过滤)"
    )
