// mock.jsx — realistic-looking session trace
// One full session: build_context → recall → score V2 → L3 LLM rerank → final 5
// Plus a second-round refine session continuation

const MOCK_SESSION_ID = "sess_2026-05-16_12-04-37_a7f1";
const MOCK_STARTED_AT = "2026-05-16 12:04:37.218";
const MOCK_TOTAL_LATENCY_MS = 2841;
...
const L2_WEIGHTS = [
  ...
  { key: "variety_bonus",    label: "variety",         w: 0.10 },
...
for (let i = 0; i < 60; i++) {
  ...
  const scores = {};
  let total = 0;
  L2_WEIGHTS.forEach(({ key, w }) => {
    const s = Math.round((r() * 1.4 - 0.2) * 100) / 100; // -0.2..1.2
    scores[key] = s;
    total += s * w;
  });
  // boost top ones a bit so the heatmap shows a gradient
...
L2_COMBOS.sort((a, b) => b.total_score - a.total_score);
L2_COMBOS.forEach((c, i) => (c.rank = i + 1));
...
// ── L3 LLM rerank ──────────────────────────────────────────
const L3 = {
  status: "ok", // ok | fallback | config_error | skipped
...
用户偏好快照 (profile snapshot):
{
  ...
  user_message: `今天午餐 · 深圳湾办公区 · 已 L2 排序的 60 个候选 (按 total_score 降序)：
...
请按工具 schema 输出 top 5，2 explore 必须覆盖至少 1 个 "wet/汤水" 类菜。`,
  ...
  raw_response_blocks: [
    {
      type: "thinking",
      text: `用户饮食方法论 = 减脂控油 + 高蛋白 (protein_floor 22g)。L2 top 5 中:
...
const FINAL = [
  ...
  { rank: 2, kind: "exploit", combo_id: "cmb_002", restaurant: "Wagas (深圳湾店)",
    ...
    dishes: [
      { name: "Mediterranean Quinoa Bowl", price: 58 },
      { name: "烟熏三文鱼牛油果", price: 78 },
    ],
    health_flags: { veg_ok: true, protein_ok: true, oil_ok: true, wetness_ok: true,
                    processed_meat: false, sweet_sauce: false },
...
// ── Refine session ─────────────────────────────────────────
const REFINE = {
  parent_session: MOCK_SESSION_ID,
  ...
  chips_to_taste_hints: {
    boost: { wetness: { wet: +0.18, soup: +0.22 }, main_ingredient_type: { fish: +0.08, poultry: +0.06 } },
    penalty: { grain_type: { noodle: -0.30, rice: -0.18, wheat: -0.12 },
...
// ── Run history ────────────────────────────────────────────
const RUN_HISTORY = [
  { id: "sess_a7f1", title: "lunch · 深圳湾",       time: "12:04", status: "ok",       latency: 2841, active: true  },
  { id: "sess_a7f0", title: "lunch · 深圳湾",       time: "12:01", status: "fallback", latency: 6210, active: false },
  { id: "sess_a7ef", title: "dinner · 家附近",       time: "昨 19:32", status: "ok",       latency: 2510, active: false },
  { id: "sess_a7ee", title: "lunch · 深圳湾",       time: "昨 12:18", status: "ok",       latency: 2620, active: false },
  { id: "sess_a7ed", title: "lunch refine · 深圳湾", time: "昨 12:21", status: "ok",       latency: 2310, active: false },
  { id: "sess_a7ec", title: "dinner · 家附近",       time: "前 19:08", status: "warn",     latency: 4520, active: false },
  { id: "sess_a7eb", title: "lunch · 深圳湾",       time: "前 11:54", status: "ok",       latency: 2402, active: false },
];

window.MOCK = {
  session_id: MOCK_SESSION_ID,
  started_at: MOCK_STARTED_AT,
  total_latency_ms: MOCK_TOTAL_LATENCY_MS,
  L1, L2_WEIGHTS, L2_COMBOS, L2_KPI,
  L3, L3_FALLBACK_EXAMPLE, FINAL, REFINE, RUN_HISTORY,
};
